...
void WeightManager::registerExpert(
    int expert_id,
    void* device_pointer,
  ...
  if (!records_.emplace(expert_id, record).second) {
    throw std::logic_error("expert is already registered");
  }
}

void WeightManager::unregisterExpert(int expert_id) {
  std::scoped_lock lock(mutex_);
  if (records_.erase(expert_id) == 0) {
...
void WeightManager::completeLoad(
...
void WeightManager::activate(int expert_id, std::size_t active_buffer) {
  std::scoped_lock lock(mutex_);
  auto it = records_.find(expert_id);
  if (it == records_.end()) {
    throw std::out_of_range("expert is not registered");
...
void WeightManager::evict(int expert_id) {
  ...
  it->second.resident_pointer = nullptr;
...
void WeightManager::publishIPC(int expert_id, void* ipc_pointer) {
  ...
  it->second.resident_pointer = ipc_pointer;
  it->second.ipc_imported = true;
  it->second.resident = true;
  it->second.state = ResidentState::kActive;
  state_changed_.notify_all();
}
...
std::vector<int> WeightManager::invalidateBuffer(std::size_t buffer_index) {
  ...
  for (auto& [id, record] : records_) {
    ...
    if (record.reference_count != 0) {
      throw std::logic_error("cannot recycle a referenced staging buffer");
    }
    record.resident = false;
    record.resident_pointer = nullptr;
    record.staging_pointer = nullptr;
    record.state = ResidentState::kEvicted;
    ids.push_back(id);
  }
  return ids;
}

void WeightManager::failLoad(int expert_id) {
  std::scoped_lock lock(mutex_);
  auto it = records_.find(expert_id);
  if (it == records_.end()) {
    return;
  ...
  it->second.resident_pointer = nullptr;
  it->second.staging_pointer = nullptr;
  // Both unloaded and evicted records are valid retry inputs for beginLoad().
  it->second.state = ResidentState::kEvicted;
  state_changed_.notify_all();
}

ExpertRecord WeightManager::getRecord(int expert_id) const {
  std::scoped_lock lock(mutex_);
...
ExpertRecord WeightManager::waitForResident(int expert_id) const {
  ...
  state_changed_.wait(lock, [&] {
    const auto it = records_.find(expert_id);
    return it == records_.end() || it->second.resident || !it->second.loading;
  });
  const auto it = records_.find(expert_id);
  if (it == records_.end()) {
    throw std::out_of_range("expert is not registered");
