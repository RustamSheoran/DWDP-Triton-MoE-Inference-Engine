  ...
  while (running_) {
    ...
    try {
      ...
      if (present.has_ipc_handle) {
        void* pointer =
            ipc_.importExpert(request->expert_id, present.ipc_handle);
      ...
      staging_.copyToNextAsync(
          record.device_pointer,
          record.size_bytes,
      ...
      for (const int victim :
           cache_.admit(request->expert_id, record.size_bytes)) {
