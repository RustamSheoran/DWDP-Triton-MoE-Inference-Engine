...
def select_fp8_dtype(device: torch.device) -> torch.dtype | None:
    ...
    for name, triton_types in candidates:
        ...
        if isinstance(dtype, torch.dtype) and any(
            hasattr(tl, type_name) for type_name in triton_types
        ):
            return dtype
    return None


def execute_persistent_qwen_fp8(
    tensors: TensorList, queues: PersistentTileQueues
) -> None:
...
if FP8_TRITON_AVAILABLE:
    ...
    @triton.jit
    def _persistent_fp8_gather_swiglu(
        queue_descriptors,
        queue_tile_m,
        queue_tile_n,
        queue_counter,
    ...
    @triton.jit
    def _persistent_fp8_down_route_store(
        queue_descriptors,
        queue_tile_m,
        queue_tile_n,
        queue_counter,
