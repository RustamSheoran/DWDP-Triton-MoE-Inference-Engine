...
pytestmark = pytest.mark.skipif(
    not torch.cuda.is_available(), reason="Triton dispatcher requires CUDA"
)
...
def assert_dispatch_plans_equal(reference_plan, triton_plan) -> None:
    assert torch.equal(
        triton_plan.assignments.expert_ids, reference_plan.assignments.expert_ids
    )
    assert torch.equal(
    ...
    assert torch.equal(
        triton_plan.metadata.expert_counts, reference_plan.metadata.expert_counts
    )
    assert torch.equal(
        triton_plan.metadata.expert_offsets, reference_plan.metadata.expert_offsets
    )
    assert torch.equal(
    ...
    assert torch.equal(
        triton_plan.metadata.destination_positions,
        reference_plan.metadata.destination_positions,
    )
    assert triton_plan.metadata.num_tokens == reference_plan.metadata.num_tokens
    assert (
        triton_plan.metadata.num_assignments == reference_plan.metadata.num_assignments
    )
...
def test_triton_dispatcher_matches_reference_random(
    num_tokens: int, num_experts: int, top_k: int
) -> None:
    generator = torch.Generator(device="cuda").manual_seed(
        1234 + num_tokens + num_experts + top_k
    )
    ...
    topk_weights = torch.rand(
        (num_tokens, top_k), dtype=torch.float32, device="cuda", generator=generator
    )
    router_output = make_router_output(topk_indices, topk_weights)
    reference_dispatcher = ExpertMajorDispatcher(
...
def test_triton_dispatcher_preserves_stable_order_edge_case() -> None:
    ...
    router_output = make_router_output(topk_indices, topk_weights)
    reference_dispatcher = ExpertMajorDispatcher(
        DispatcherConfig(num_experts=4, algorithm="counting_scatter")
    )
    triton_dispatcher = ExpertMajorDispatcher(
        DispatcherConfig(num_experts=4, algorithm="triton_counting_scatter")
    )
...
def test_triton_dispatcher_reuses_workspace() -> None:
    topk_indices = torch.tensor(
        [[2, 1], [0, 2], [1, 0]], dtype=torch.int64, device="cuda"
    )
    topk_weights = torch.ones_like(topk_indices, dtype=torch.float32)
    router_output = make_router_output(topk_indices, topk_weights)
    dispatcher = ExpertMajorDispatcher(
        DispatcherConfig(num_experts=3, algorithm="triton_counting_scatter")
    )
    ...
    assert (
        second.metadata.token_permutation.data_ptr()
        == first.metadata.token_permutation.data_ptr()
    )
    assert (
        second.assignments.packed_routing_weights.data_ptr()
        == first.assignments.packed_routing_weights.data_ptr()
    )
...
def test_triton_dispatcher_reuses_router_histogram_metadata() -> None:
    topk_indices = torch.tensor(
        [[3, 1], [3, 2], [1, 0], [0, 3]], dtype=torch.int64, device="cuda"
    )
    ...
    dispatcher = ExpertMajorDispatcher(
        DispatcherConfig(
            num_experts=4,
            algorithm="triton_counting_scatter",
            reuse_router_metadata=True,
        )
    )
...
def test_triton_dispatcher_rejects_unstable_order() -> None:
    with pytest.raises(ValueError):
        DispatcherConfig(
            num_experts=4, algorithm="triton_counting_scatter", stable_order=False
        )
