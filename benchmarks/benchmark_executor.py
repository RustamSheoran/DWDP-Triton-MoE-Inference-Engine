...
def make_plans(
    num_tokens: int, num_experts: int, device: str
) -> tuple[DispatchPlan, ExecutionPlan, CommunicationPlan]:
    counts = torch.full(
        (num_experts,), num_tokens // num_experts, dtype=torch.int64, device=device
    )
    counts[: num_tokens % num_experts] += 1
    offsets = torch.cat((counts.new_zeros(1), counts.cumsum(dim=0)), dim=0)
    expert_ids = torch.repeat_interleave(
        torch.arange(num_experts, dtype=torch.int64, device=device), counts
    )
    ...
    dispatch_plan = DispatchPlan(
        ...
        metadata=DispatchMetadata(
            ...
            inverse_permutation=torch.arange(
                num_tokens, dtype=torch.int64, device=device
            ),
    ...
    execution_plan = ExecutionPlan(
        ...
        stream_assignments=torch.zeros(
            active.numel(), dtype=torch.int64, device=device
        ),
        batches=(),
        synchronization=SchedulerSynchronizationMetadata(
            torch.zeros(active.numel(), dtype=torch.bool, device=device)
        ),
        ...
        statistics=SchedulerStatistics(
            num_experts,
            active.numel(),
            num_experts - active.numel(),
            active.numel(),
            num_tokens,
            int(counts.max().item()),
            int(counts[counts > 0].min().item()),
            "round_robin",
        ),
    ...
    communication_plan = CommunicationPlan(
        ...
        statistics=CommunicationStatistics(
            active.numel(), 0, 0, 0, 0, 0, 0, 0.0, "static"
        ),
...
def main() -> None:
    ...
    dispatch_plan, execution_plan, communication_plan = make_plans(
        args.tokens, args.num_experts, args.device
    )
    experts = ExpertRegistry(
        [
            MLPExpert(args.hidden_size, args.intermediate_size).to(args.device)
    ...
    output_bytes = estimate_tensor_bytes(
        output.weighted_expert_outputs
    ) + estimate_tensor_bytes(output.packed_expert_outputs)
    print(f"device={args.device}")
    print("backend=pytorch")
    print(
        f"tokens={args.tokens} hidden_size={args.hidden_size} experts={args.num_experts}"
    )
